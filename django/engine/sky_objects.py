...
Uses skyfield for ephemeris calculations and the bundled Messier catalog.
"""
import logging
import math
...
from .messier import load_catalog
...
# Approximate apparent magnitudes at mean opposition (used as default when
# skyfield magnitude computation is unavailable for older ephemeris versions)
_PLANET_MEAN_MAG = {
    "Mercury": 0.2,
    "Venus":   -4.4,
    "Mars":    0.5,
...
METEOR_SHOWERS = [
    {"name": "Quadrantids",    "peak_month": 1,  "peak_day": 3,  "zhr": 120, "ra_h": 15.3, "dec_d": 49.5},
    {"name": "Lyrids",         "peak_month": 4,  "peak_day": 22, "zhr": 18,  "ra_h": 18.2, "dec_d": 34.0},
    {"name": "Eta Aquariids",  "peak_month": 5,  "peak_day": 6,  "zhr": 50,  "ra_h": 22.5, "dec_d": -1.0},
    {"name": "Delta Aquariids","peak_month": 7,  "peak_day": 28, "zhr": 20,  "ra_h": 22.5, "dec_d": -16.0},
    {"name": "Perseids",       "peak_month": 8,  "peak_day": 12, "zhr": 100, "ra_h": 3.1,  "dec_d": 58.0},
    {"name": "Draconids",      "peak_month": 10, "peak_day": 8,  "zhr": 10,  "ra_h": 17.5, "dec_d": 54.0},
    {"name": "Orionids",       "peak_month": 10, "peak_day": 21, "zhr": 25,  "ra_h": 6.3,  "dec_d": 15.6},
    {"name": "Taurids",        "peak_month": 11, "peak_day": 5,  "zhr": 5,   "ra_h": 3.7,  "dec_d": 14.0},
    {"name": "Leonids",        "peak_month": 11, "peak_day": 17, "zhr": 15,  "ra_h": 10.1, "dec_d": 21.5},
    {"name": "Geminids",       "peak_month": 12, "peak_day": 13, "zhr": 150, "ra_h": 7.4,  "dec_d": 32.5},
    {"name": "Ursids",         "peak_month": 12, "peak_day": 22, "zhr": 10,  "ra_h": 14.5, "dec_d": 75.3},
]
...
def _night_window(eph, ts, observer, target_date: date):
    ...
    # Fallback: 9 PM to 5 AM local
    if night_start is None:
        night_start = ts.utc(target_date.year, target_date.month, target_date.day, 1)  # 1 UTC ~8p CDT
    if night_end is None:
        night_end = ts.utc(next_day.year, next_day.month, next_day.day, 11)  # 11 UTC ~6a CDT
...
def _fetch_iss_tle() -> tuple[str, str] | None:
    """Fetch ISS TLE lines from Celestrak. Returns (line1, line2) or None."""
    from django.core.cache import cache
    cached = cache.get("iss:tle")
    if cached:
        return cached
    try:
        resp = requests.get(_ISS_TLE_URL, timeout=5)
        ...
        if len(lines) >= 3:
            ...
            return None
        cache.set("iss:tle", result, 6 * 3600)
        return result
    except Exception as e:
        _log.debug("ISS TLE fetch failed: %s", e)
        return None


def _compute_iss_passes(lat: float, lon: float, t0, t1, ts, tz: ZoneInfo) -> list[dict]:
    """Return list of ISS pass dicts for the night window."""
...
def compute_sky(
    ...
    # ── Planets ──────────────────────────────────────────────────────────────
    planets = []
    for name, body_key in _PLANET_BODIES.items():
        ...
        # Check if ever above min altitude during night
        above = alts > _MIN_ALTITUDE
        if not np.any(above):
            planets.append({
                "name": name, "visible": False,
                "note": "Below horizon tonight",
            })
            continue

        # Rise/set within night window
        p_rise, p_set = _rise_set(eph, planet_obj, observer, t_start, t_end)

        # Apparent magnitude
        try:
            from skyfield.magnitudes import planetary_magnitude
            t_mid_planet = times[peak_idx]
            astr = observer.at(t_mid_planet).observe(planet_obj)
            app = astr.apparent()
            mag = round(planetary_magnitude(app), 1)
        except Exception:
            mag = _PLANET_MEAN_MAG.get(name, 5.0)

        # Apply extinction penalty if low in sky
    ...
    # Sort: visible first (by tier), then not visible
    planets.sort(key=lambda p: (not p["visible"], p.get("tier", 4), -p.get("peak_alt", 0)))

    # ── DSOs (Messier) ───────────────────────────────────────────────────────
    catalog = load_catalog()
    dsos = []
    dsos_drive = []  # visible from dark sky but not from current Bortle
    for obj in catalog:
        star = Star(ra_hours=obj["ra_h"], dec_degrees=obj["dec_d"])
        times, alts, _ = _altaz_series(observer, star, t_start, t_end, ts)
        above = alts > _MIN_ALTITUDE
        if not np.any(above):
            continue
        ...
        if tier < 4:
            ...
            # Would be visible with large scope from a darker site (Bortle ≤ 3)
            dark_limit = BORTLE_LIMITING_MAG[3] + EQUIPMENT_GAIN["large_scope"]
            if mag <= dark_limit:
                dsos_drive.append(entry)

    # Sort by tier then peak altitude
    ...
    for shower in METEOR_SHOWERS:
        days = _days_to_peak(target_date, shower["peak_month"], shower["peak_day"])
        if abs(days) > 14:
            continue

        # Check if radiant is above horizon during night
        radiant = Star(ra_hours=shower["ra_h"], dec_degrees=shower["dec_d"])
    ...
    # ── ISS ──────────────────────────────────────────────────────────────────
    iss_passes = _compute_iss_passes(lat, lon, t_start, t_end, ts, tz)
